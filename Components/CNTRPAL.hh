#ifndef R1000_CNTRPAL
#define R1000_CNTRPAL

struct scm_cntrpal_state;

SC_MODULE(SCM_CNTRPAL)
{
	...
	SC_HAS_PROCESS(SCM_CNTRPAL);

	SCM_CNTRPAL(sc_module_name nm, const char *arg) : sc_module(nm)
	{
		SC_METHOD(doit);
		sensitive << pin1.pos();
		loadit(arg);
	...
	private:
	struct scm_cntrpal_state *state;
